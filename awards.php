...
?>
<?php
// Fetch only the cli_awards and org_awards categories
$categories_sql = "SELECT * FROM categories WHERE slug IN ('cli_awards', 'org_awards') ORDER BY name";
...
if (empty($categories_list)) {
    $categories_list = [
        ['id' => 'cli_awards', 'name' => 'Client', 'slug' => 'cli_awards'],
        ['id' => 'org_awards', 'name' => 'Organization', 'slug' => 'org_awards']
    ];
}
...
// Filter by specified category if provided, otherwise filter by both award categories
if ($filter_category && is_numeric($filter_category)) {
    ...
    $sql .= " AND c.slug IN ('cli_awards', 'org_awards')";
}
...
$stmt->execute($params);
$award_contents = $stmt->fetchAll();

// Determine the page title based on the filter
$dynamic_page_title = "Awards & Recognition"; // Default title
if ($filter_category) {
    foreach ($categories_list as $cat) {
        if ($cat['id'] == $filter_category) {
            $dynamic_page_title = sanitize($cat['name']); // Remove the redundant "Awards" suffix
            break;
        }
    }
}
...
<main>
    <section class="news-grid-section">
        ...
        <div class="contents-grid">
            <?php if (count($award_contents) > 0): ?>
                <?php foreach ($award_contents as $art): ?>
                    <?php
                    if ($art['thumbnail']) {
            ...
            <?php else: ?>
                <div class="no-awards-message">
                    <p>No awards or recognitions found in this category.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
