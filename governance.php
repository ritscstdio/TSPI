...
$page_description = "TSPI Corporate Governance Documents";
$body_class = "governance-page";
include 'includes/header.php';
?>
<?php
// Fetch only the corporate governance categories
$categories_sql = "SELECT * FROM categories WHERE slug IN ('leg_documents', 'reg_registrations', 'gov_framework') ORDER BY name";
...
if (empty($categories_list)) {
    $categories_list = [
        ['id' => 'leg_documents', 'name' => 'Foundational Legal Documents', 'slug' => 'leg_documents'],
        ['id' => 'reg_registrations', 'name' => 'Regulatory Compliance', 'slug' => 'reg_registrations'],
        ['id' => 'gov_framework', 'name' => 'Governance Framework', 'slug' => 'gov_framework']
    ];
}
...
// Filter by specified category if provided, otherwise filter by all governance categories
if ($filter_category && is_numeric($filter_category)) {
    ...
    $sql .= " AND c.slug IN ('leg_documents', 'reg_registrations', 'gov_framework')";
}
...
$stmt->execute($params);
$governance_articles = $stmt->fetchAll();

// Determine the page title based on the filter
$dynamic_page_title = "Corporate Governance"; // Default title
if ($filter_category) {
    foreach ($categories_list as $cat) {
        if ($cat['id'] == $filter_category) {
    ...
    <section class="news-grid-section">
        ...
        <form method="get" class="news-filters">
            ...
            <select id="filter-category" name="category" onchange="this.form.submit()">
                <option value="">All Governance Documents</option>
        ...
        <div class="articles-grid">
            <?php if (count($governance_articles) > 0): ?>
                <?php foreach ($governance_articles as $art): ?>
                    <?php
                    if ($art['thumbnail']) {
            ...
            <?php else: ?>
                <div class="no-articles-message">
                    <p>No governance documents found in this category.</p>
                </div>
            <?php endif; ?>
        </div>
    </section>
